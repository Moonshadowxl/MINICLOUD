...
export class Storage {
  ...
  private pruneVersions(fileId: string): void {
    const stale = this.db
      .prepare(
    ...
    for (const v of stale) {
      ...
      this.staleBlobs.push(v.blob_id);
    }
  }

  /** Blob ids whose disk data should be removed; callers may await cleanupStaleBlobs(). */
  private staleBlobs: string[] = [];

  async cleanupStaleBlobs(userId: string): Promise<void> {
    const ids = this.staleBlobs.splice(0);
    await Promise.all(ids.map((id) => this.deleteBlobData(userId, id)));
  }
  ...
  ensureParentDirs(userId: string, p: string, t: number): void {
    ...
    for (let i = 0; i < parts.length - 1; i++) {
      ...
      const existing = this.db
        .prepare('SELECT id, is_dir FROM files WHERE user_id = ? AND path = ? AND deleted_at IS NULL')
        .get(userId, cur) as { id: string; is_dir: number } | undefined;
      if (existing) {
      ...
      this.db
        ...
        .run(randomId(), userId, cur, parts[i], t);
    }
  }
  ...
  mkdir(userId: string, rawPath: string): FileRow {
    ...
    if (!p) throw new StorageError(400, 'path required');
    const t = now();
    this.ensureParentDirs(userId, `${p}/x`, t); // creates p itself and its parents
  ...
  list(userId: string, rawPath: string): FileRow[] {
    ...
    return this.db
      ...
      .all(userId, like, depth - 1) as unknown as FileRow[];
  }
  ...
  trash(userId: string, rawPath: string): void {
    ...
    try {
      ...
      if (f.is_dir) {
        this.db
          .prepare('UPDATE files SET deleted_at = ? WHERE user_id = ? AND deleted_at IS NULL AND path LIKE ?')
          .run(t, userId, `${p}/%`);
      }
      this.db.exec('COMMIT');
    } catch (err) {
  ...
  restore(userId: string, fileId: string): void {
    ...
    try {
      ...
      if (f.is_dir) {
        this.db
          .prepare('UPDATE files SET deleted_at = NULL WHERE user_id = ? AND deleted_at = ? AND path LIKE ?')
          .run(userId, f.deleted_at, `${f.path}/%`);
      }
      this.db.exec('COMMIT');
    } catch (err) {
      this.db.exec('ROLLBACK');
  ...
  usage(userId: string) {
    ...
    for (const app of apps) {
      const sub = this.db
        .prepare(
          `SELECT category, COALESCE(SUM(size), 0) AS bytes FROM files
           WHERE user_id = ? AND is_dir = 0 AND deleted_at IS NULL AND (path = ? OR path LIKE ?)
             AND category != 'apps'
           GROUP BY category`,
        )
        .all(userId, app.root_path, `${app.root_path}/%`) as { category: string; bytes: number }[];
      for (const r of sub) {
